...
from market_impact.function_form import (
    scaling_form,
    scaling_law,
)
...
def plot_scaling_form(
    ...
    for T, param in fit_parameters.items():
        ...
        binned_data = bin_data_into_quantiles(data, q=q, duplicates="drop", x_col=x_col)
...
def plot_scaling_law(rescaled_parameters: Dict, q: int, x_col="volume_imbalance"):
    """
    Plots the scaling law where the scaling functions collapses onto a single master curve after renormalization.
    """
    fig = plt.figure(figsize=(4.5, 4))
    ax = fig.gca()
    ...
    for T, rescaled_param in rescaled_parameters.items():
        ...
        if x_col == "volume_imbalance":
            ...
            xlabel = r"$\it{\Delta \mathcal{E}^\prime/\mathcal{E}_{D}T^{\varkappa}}$"
            ylabel = r"$\it{R(\Delta \mathcal{E}^\prime,T)/\mathcal{R}(1)T^{\chi}}$"
        else:
            xlabel = x_col
            ylabel = r"\it{R}"
        plt.xlabel(xlabel, fontsize=10)
